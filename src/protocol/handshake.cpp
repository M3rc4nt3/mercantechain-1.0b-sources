...
bool MercanteNode_DisconnectRemote(CNode *pnode)
{
    ...
    {
        return !MercanteNode_CanConnect(pnode);
    }
...
bool MercanteNode_AcceptData(CNode *pnode)
{
    ...
    return !MercanteNode_DisconnectRemote(pnode);
}
...
bool VerifyMercanteVerackHash(string sParameterSetHash,uint64_t nNonce)
{
...
{   
    ...
    {
        if(!VerifyMercanteVerackHash(sParameterSetHash,nNonce))
        {
            if(!fIsVerackack)
            {
...
//                return false;
    ...
    if(MCP_ANYONE_CAN_CONNECT == 0)
    {
        ...
        if(fIsVerackack)
        {
            ...
            if(!MercanteNode_CanConnect(pfrom))
            {
