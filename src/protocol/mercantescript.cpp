...
#define MC_DCT_SCRIPT_MERCANTE_IDENTIFIER    "spk"
#define MC_DCT_SCRIPT_IDENTIFIER_LEN 3

#define MC_DCT_SCRIPT_MERCANTE_ENTITY_PREFIX 'e'
#define MC_DCT_SCRIPT_MERCANTE_CASHED_SCRIPT_PREFIX 'i'
#define MC_DCT_SCRIPT_MERCANTE_KEY_PREFIX 'k'
#define MC_DCT_SCRIPT_MERCANTE_APPROVE_PREFIX 'a'
#define MC_DCT_SCRIPT_MERCANTE_NEW_ENTITY_PREFIX 'n'
#define MC_DCT_SCRIPT_MERCANTE_UPDATE_ENTITY_PREFIX 'u'
#define MC_DCT_SCRIPT_MERCANTE_PERMISSIONS_PREFIX 'p'
#define MC_DCT_SCRIPT_MERCANTE_BLOCK_SIGNATURE_PREFIX 'b'
#define MC_DCT_SCRIPT_MERCANTE_ASSET_GENESIS_PREFIX 'g'
#define MC_DCT_SCRIPT_MERCANTE_ASSET_QUANTITY_PREFIX 'q'
#define MC_DCT_SCRIPT_MERCANTE_ASSET_DETAILS_PREFIX 'a'
#define MC_DCT_SCRIPT_MERCANTE_ASSET_FOLLOWON_PREFIX 'o'
#define MC_DCT_SCRIPT_MERCANTE_GENERAL_DETAILS_PREFIX 'c'
#define MC_DCT_SCRIPT_MERCANTE_DATA_FORMAT_PREFIX 'f'
#define MC_DCT_SCRIPT_MERCANTE_RAW_DATA_PREFIX 'd'
...
int mc_Script::GetPermission(uint32_t *type,uint32_t *from,uint32_t *to,uint32_t *timestamp)
{
    ...
    if(memcmp(ptr,MC_DCT_SCRIPT_MERCANTE_IDENTIFIER,MC_DCT_SCRIPT_IDENTIFIER_LEN) != 0)
    {
        return MC_ERR_WRONG_SCRIPT;
    }
    
    if(ptr[MC_DCT_SCRIPT_IDENTIFIER_LEN] != MC_DCT_SCRIPT_MERCANTE_PERMISSIONS_PREFIX)
    {
        return MC_ERR_WRONG_SCRIPT;
    }
...
int mc_Script::SetPermission(uint32_t type,uint32_t from,uint32_t to,uint32_t timestamp)
{
    ...
    ptr=buf;
    memcpy(ptr,MC_DCT_SCRIPT_MERCANTE_IDENTIFIER,MC_DCT_SCRIPT_IDENTIFIER_LEN);
    ptr[MC_DCT_SCRIPT_IDENTIFIER_LEN]=MC_DCT_SCRIPT_MERCANTE_PERMISSIONS_PREFIX;
...
int mc_Script::GetBlockSignature(unsigned char* sig,int *sig_size,uint32_t* hash_type,unsigned char* key,int *key_size)
{
    ...
    if(ptr[MC_DCT_SCRIPT_IDENTIFIER_LEN] != MC_DCT_SCRIPT_MERCANTE_BLOCK_SIGNATURE_PREFIX)
    {
        return MC_ERR_WRONG_SCRIPT;
    }
...
int mc_Script::SetBlockSignature(const unsigned char* sig,int sig_size,uint32_t hash_type,const unsigned char* key,int key_size)
{
    ...
    buf[MC_DCT_SCRIPT_IDENTIFIER_LEN]=MC_DCT_SCRIPT_MERCANTE_BLOCK_SIGNATURE_PREFIX;
...
int mc_Script::GetAssetGenesis(int64_t *quantity)
{
    ...
    if(ptr[MC_DCT_SCRIPT_IDENTIFIER_LEN] != MC_DCT_SCRIPT_MERCANTE_ASSET_GENESIS_PREFIX)
    {
        return MC_ERR_WRONG_SCRIPT;
    }
...
int mc_Script::SetAssetGenesis(int64_t quantity)
{
    ...
    ptr=buf;
    memcpy(ptr,MC_DCT_SCRIPT_MERCANTE_IDENTIFIER,MC_DCT_SCRIPT_IDENTIFIER_LEN);
    ptr[MC_DCT_SCRIPT_IDENTIFIER_LEN]=MC_DCT_SCRIPT_MERCANTE_ASSET_GENESIS_PREFIX;
...
int mc_Script::GetAssetDetails(char* name,int* multiple,unsigned char* script,int *script_size)
{
    ...
    if(ptr[MC_DCT_SCRIPT_IDENTIFIER_LEN] != MC_DCT_SCRIPT_MERCANTE_ASSET_DETAILS_PREFIX)
    {
        return MC_ERR_WRONG_SCRIPT;
    }
...
int mc_Script::SetAssetDetails(const char*name,int multiple,const unsigned char* script,int script_size)
{
    ...
    memcpy(buf,MC_DCT_SCRIPT_FREE_DATA_IDENTIFIER,MC_DCT_SCRIPT_IDENTIFIER_LEN);
    buf[MC_DCT_SCRIPT_IDENTIFIER_LEN]=MC_DCT_SCRIPT_MERCANTE_ASSET_DETAILS_PREFIX;
...
int mc_Script::GetGeneralDetails(unsigned char* script,int *script_size)
{
    ...
    if(ptr[MC_DCT_SCRIPT_IDENTIFIER_LEN] != MC_DCT_SCRIPT_MERCANTE_GENERAL_DETAILS_PREFIX)
    {
        return MC_ERR_WRONG_SCRIPT;
    }
...
int mc_Script::SetGeneralDetails(const unsigned char* script,int script_size)
{
    ...
    memcpy(buf,MC_DCT_SCRIPT_FREE_DATA_IDENTIFIER,MC_DCT_SCRIPT_IDENTIFIER_LEN);
    buf[MC_DCT_SCRIPT_IDENTIFIER_LEN]=MC_DCT_SCRIPT_MERCANTE_GENERAL_DETAILS_PREFIX;
...
int mc_Script::GetApproval(uint32_t *approval,uint32_t *timestamp)
{
    ...
    if(ptr[MC_DCT_SCRIPT_IDENTIFIER_LEN] != MC_DCT_SCRIPT_MERCANTE_APPROVE_PREFIX)
    {
        return MC_ERR_WRONG_SCRIPT;
    }
...
int mc_Script::SetApproval(uint32_t approval,uint32_t timestamp)
{
    ...
    ptr=buf;
    memcpy(ptr,MC_DCT_SCRIPT_MERCANTE_IDENTIFIER,MC_DCT_SCRIPT_IDENTIFIER_LEN);
    ptr[MC_DCT_SCRIPT_IDENTIFIER_LEN]=MC_DCT_SCRIPT_MERCANTE_APPROVE_PREFIX;
...
int mc_Script::GetNewEntityType(uint32_t *type,int *update,unsigned char* script,int *script_size)
{
    ...
    if(ptr[MC_DCT_SCRIPT_IDENTIFIER_LEN] == MC_DCT_SCRIPT_MERCANTE_NEW_ENTITY_PREFIX)
    {
        *update=0;
    }
    else
    {
        if(ptr[MC_DCT_SCRIPT_IDENTIFIER_LEN] == MC_DCT_SCRIPT_MERCANTE_UPDATE_ENTITY_PREFIX)
        {
            *update=1;
        }
        else
...
int mc_Script::SetNewEntityType(const uint32_t type,const int update,const unsigned char* script,int script_size)
{
    ...
    memcpy(buf,MC_DCT_SCRIPT_MERCANTE_IDENTIFIER,MC_DCT_SCRIPT_IDENTIFIER_LEN);
    if(update)
    {
        buf[MC_DCT_SCRIPT_IDENTIFIER_LEN]=MC_DCT_SCRIPT_MERCANTE_UPDATE_ENTITY_PREFIX;        
    }
    else
    {
        buf[MC_DCT_SCRIPT_IDENTIFIER_LEN]=MC_DCT_SCRIPT_MERCANTE_NEW_ENTITY_PREFIX;
    }
...
int mc_Script::GetItemKey(unsigned char *key,int *key_size)
{
    ...
    if(ptr[MC_DCT_SCRIPT_IDENTIFIER_LEN] != MC_DCT_SCRIPT_MERCANTE_KEY_PREFIX)
    {
        return MC_ERR_WRONG_SCRIPT;
    }
...
int mc_Script::SetItemKey(const unsigned char* key,int key_size)
{
    ...
    memcpy(buf,MC_DCT_SCRIPT_MERCANTE_IDENTIFIER,MC_DCT_SCRIPT_IDENTIFIER_LEN);
    buf[MC_DCT_SCRIPT_IDENTIFIER_LEN]=MC_DCT_SCRIPT_MERCANTE_KEY_PREFIX;
...
int mc_Script::GetEntity(unsigned char *short_txid)
{
    ...
    if(ptr[MC_DCT_SCRIPT_IDENTIFIER_LEN] != MC_DCT_SCRIPT_MERCANTE_ENTITY_PREFIX)
    {
        return MC_ERR_WRONG_SCRIPT;
    }
    ptr+=MC_DCT_SCRIPT_IDENTIFIER_LEN+1;
...
int mc_Script::SetEntity(const unsigned char *short_txid)
{
    ...
    memcpy(buf,MC_DCT_SCRIPT_MERCANTE_IDENTIFIER,MC_DCT_SCRIPT_IDENTIFIER_LEN);
    buf[MC_DCT_SCRIPT_IDENTIFIER_LEN]=MC_DCT_SCRIPT_MERCANTE_ENTITY_PREFIX;
...
int mc_Script::GetFullRef(unsigned char *ref,uint32_t *script_type)
{
    ...
    *script_type=0;
    ...
    if(ptr[MC_DCT_SCRIPT_IDENTIFIER_LEN] == MC_DCT_SCRIPT_MERCANTE_ASSET_FOLLOWON_PREFIX)
    {
...
int mc_Script::GetAssetQuantities(mc_Buffer *amounts,uint32_t script_type)
{
    ...
    {
        if(ptr[MC_DCT_SCRIPT_IDENTIFIER_LEN] == MC_DCT_SCRIPT_MERCANTE_ASSET_QUANTITY_PREFIX)
        {
            valid_identitfier=1;
...
int mc_Script::SetAssetQuantities(mc_Buffer *amounts,uint32_t script_type)
{
    ...
    switch(script_type)
    {
        case MC_SCR_ASSET_SCRIPT_TYPE_TRANSFER:
            ptr[MC_DCT_SCRIPT_IDENTIFIER_LEN]=MC_DCT_SCRIPT_MERCANTE_ASSET_QUANTITY_PREFIX;
            break;
        case MC_SCR_ASSET_SCRIPT_TYPE_FOLLOWON:
            ptr[MC_DCT_SCRIPT_IDENTIFIER_LEN]=MC_DCT_SCRIPT_MERCANTE_ASSET_FOLLOWON_PREFIX;
            break;
        default:
            return MC_ERR_INVALID_PARAMETER_VALUE;
    }
...
int mc_Script::GetCachedScript(int offset, int *next_offset, int* vin, unsigned char** script, int *script_size)
{
    ...
    if(ptr[MC_DCT_SCRIPT_IDENTIFIER_LEN] != MC_DCT_SCRIPT_MERCANTE_CASHED_SCRIPT_PREFIX)
    {
        return MC_ERR_WRONG_SCRIPT;
    }
...
int mc_Script::SetCachedScript(int offset, int *next_offset, int vin, unsigned char* script, int script_size)
{
    ...
    if(offset == 0)
    {
        ...
        ptr[MC_DCT_SCRIPT_IDENTIFIER_LEN]=MC_DCT_SCRIPT_MERCANTE_CASHED_SCRIPT_PREFIX;
...
int mc_Script::GetRawData(unsigned char **data,int *size)
{
    ...
    if(ptr[MC_DCT_SCRIPT_IDENTIFIER_LEN] != MC_DCT_SCRIPT_MERCANTE_RAW_DATA_PREFIX)
    {
        return MC_ERR_WRONG_SCRIPT;            
    }
...
int mc_Script::SetRawData(const unsigned char *data,const int size)
{
    ...
    memcpy(buf,MC_DCT_SCRIPT_MERCANTE_IDENTIFIER,MC_DCT_SCRIPT_IDENTIFIER_LEN);
    buf[MC_DCT_SCRIPT_IDENTIFIER_LEN]=MC_DCT_SCRIPT_MERCANTE_RAW_DATA_PREFIX;        
...
int mc_Script::GetDataFormat(uint32_t *format)
{
    ...
    if(ptr[MC_DCT_SCRIPT_IDENTIFIER_LEN] != MC_DCT_SCRIPT_MERCANTE_DATA_FORMAT_PREFIX)
    {
        return MC_ERR_WRONG_SCRIPT;            
    }
...
int mc_Script::SetDataFormat(const uint32_t format)
{
    ...
    memcpy(buf,MC_DCT_SCRIPT_MERCANTE_IDENTIFIER,MC_DCT_SCRIPT_IDENTIFIER_LEN);
    buf[MC_DCT_SCRIPT_IDENTIFIER_LEN]=MC_DCT_SCRIPT_MERCANTE_DATA_FORMAT_PREFIX;        
