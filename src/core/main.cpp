...
bool AcceptMercanteTransaction(const CTransaction& tx, 
...
bool ProcessMercanteVerack(CNode* pfrom, CDataStream& vRecv,bool fIsVerackack,bool *disconnect_flag);
bool PushMercanteVerack(CNode* pfrom, bool fIsVerackack);
bool MercanteNode_CanConnect(CNode *pnode);
bool MercanteNode_DisconnectRemote(CNode *pnode);
bool MercanteNode_DisconnectLocal(CNode *pnode);
bool MercanteNode_RespondToGetData(CNode *pnode);
bool MercanteNode_SendInv(CNode *pnode);
bool MercanteNode_AcceptData(CNode *pnode);
bool MercanteNode_IgnoreIncoming(CNode *pnode);
bool MercanteNode_IsLocal(CNode *pnode);
...
const string strMessageMagic = "Mercante Signed Message:\n";
...
int SetUpgradedParamValue(const mc_OneMercanteParam *param,int64_t value)
{
    ...
    if(strcmp(param->m_Name,"targetblocktime") == 0)
    {
        ...
        SetMercanteParam("targetblocktime",value);
    }   
...
int MercanteNode_ApplyUpgrades(int current_height)
{
...
int MercanteNode_ApplyUpgrades_Old(int current_height)
{
...
void MercanteNode_UpdateBlockByHeightList(CBlockIndex *pindex)
{
    if(pindex->nHeight < 0)
    {
        return;
...
bool MercanteNode_IsBlockChainSynced(CNode *pnode)
{
...
/* MCHN START */
        ...
        if(mc_gState->m_Features->Streams())
        {
            if(!AcceptMercanteTransaction(tx,view,-1,true,reason, &replay))
            {
                return state.DoS(0,
                                 error("AcceptToMemoryPool: : AcceptMercanteTransaction failed %s : %s", hash.ToString(),reason),
                                 REJECT_NONSTANDARD, reason);
            }
        }
        else
...
/* MCHN START */        
    if (block.GetHash() == Params().HashGenesisBlock()){        
        ...
        for (unsigned int i = 0; i < block.vtx.size(); i++)
        {
            ...
            if(mc_gState->m_Features->Streams())
            {
                if(!AcceptMercanteTransaction(tx,view,offset,true,reason,NULL))
                {
...
/* MCHN START */        
            ...
            if(!fJustCheck)
            {
                if(mc_gState->m_Features->Streams())
                {
                    ...
                    {
                        return state.DoS(0,
                                         error("ConnectBlock: : AcceptMercanteTransaction failed %s : %s", tx.GetHash().ToString(),reason),
                                         REJECT_NONSTANDARD, reason);
                    }
                }
                else
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (tx.IsCoinBase())
        {
            ...
            if(!fJustCheck)
            {
                if(mc_gState->m_Features->Streams())
                {
                    if(!AcceptMercanteTransaction(tx,view,coinbase_offset,true,reason,NULL))
                    {
                        return false;       
                    }
                }
...
/* MCHN START */    
    ...
    MercanteNode_ApplyUpgrades(old_height-1);        
    if(mc_gState->m_WalletMode & MC_WMD_TXS)
    {
...
/* MCHN START */        
    ...
    MercanteNode_ApplyUpgrades(chainActive.Height());    
/* MCHN END */    
...
CBlockIndex* AddToBlockIndex(const CBlockHeader& block)
{
    ...
    MercanteNode_UpdateBlockByHeightList(pindexNew);
...
bool AcceptBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex** ppindex, int node_id, CBlockIndex *pindexChecked)
{
    ...
    if( (node_id > 0) && (pindexPrev != NULL) )
    {
        ...
        {
            MercanteNode_UpdateBlockByHeightList(pindexPrev);
        }
        lpNextForPrev=NULL;
...
/* MCHN START */    
    if(!MercanteNode_RespondToGetData(pfrom)) 
    {
...
/* MCHN START */
        ...
        {            
            ...
            {
                PushMercanteVerack(pfrom,false);
            }
        }
        else
        {
...
/* MCHN START */
        if(mc_gState->m_NetworkParams->IsProtocolMercante())
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if(!ProcessMercanteVerack(pfrom,vRecv,false,&disconnect_flag))
                    {
                    ...
                    else
                    {
                        ...
                        {
                            pfrom->fDisconnect = !PushMercanteVerack(pfrom,true);
                        }
                        else
                        {
    ...
    else if (strCommand == "verackack")
    {
        if(mc_gState->m_NetworkParams->IsProtocolMercante())
        {
            ...
            if(!ProcessMercanteVerack(pfrom,vRecv,true,&disconnect_flag))
            {
            ...
            {
                pfrom->fDisconnect |= !PushMercanteVerack(pfrom,false);       // MCHN |= to avoid setting fDisconnect to false
            }
            if(pfrom->fDisconnect)
            {
                mc_gState->m_NetworkState = MC_NTS_SEED_READY;
...
/* MCHN END */
    ...
    else if (strCommand == "inv")
    {
        ...
        for (unsigned int nInv = 0; nInv < vInv.size(); nInv++)
        {
            ...
            if (inv.type == MSG_BLOCK) {
                ...
                if (!fAlreadyHave && !fImporting && !fReindex && !mapBlocksInFlight.count(inv.hash)) {
                    ...
                    if(!MercanteNode_IgnoreIncoming(pfrom))
                    {
        ...
        if (!vToFetch.empty() && !MercanteNode_IgnoreIncoming(pfrom))      // MCHN
    ...
    else if (strCommand == "tx" && MercanteNode_AcceptData(pfrom))            // MCHN
    {
/* MCHN START */        
        if(!MercanteNode_IgnoreIncoming(pfrom))
        {
    ...
    else if (strCommand == "headers" && !fImporting && !fReindex) // Ignore headers received while importing
    {
        ...
        if (nCount == MAX_HEADERS_RESULTS && pindexLast) {
            ...
            pfrom->PushMessage("getheaders", chainActive.GetLocator(pindexLast), uint256(0));
                                                                                ...
                                                                                // MCHN
    else if (strCommand == "block" && !fImporting && !fReindex && MercanteNode_AcceptData(pfrom)) // Ignore blocks received while importing
    {
        CBlock block;
        vRecv >> block;
/* MCHN START */        
...
/* MCHN END */        
    ...
    else if (strCommand == "mempool" && MercanteNode_SendInv(pfrom))          // MCHN
    {
...
/* MCHN START */    
    {        
        ...
        if(!pfrom->fDisconnect)
        {
            if(MercanteNode_DisconnectRemote(pfrom))
            {            
                    pfrom->fDisconnect=true;
            ...
            if(!pfrom->fDisconnect)
            {
                if(pfrom->fCanConnectLocal && MercanteNode_DisconnectLocal(pfrom))
                {
                        pfrom->fDisconnect=true;
...
/* MCHN END */            
        ...
        {
/* MCHN START */            
            if(pfrom->fDisconnect || !MercanteNode_DisconnectRemote(pfrom))
            {
/* MCHN END */            
...
*/
                ...
                if (pto->setInventoryKnown.insert(inv).second)
                {
                    ...
                    if (vInv.size() >= 1000)
                    {
                        if(MercanteNode_SendInv(pto))                         // MCNN
...
/* MCHN START */        
        ...
        {
            LOCK(cs_main);
            ignore_incoming=MercanteNode_IgnoreIncoming(pto);
        }
        if(!ignore_incoming)
        {
...
/* MCHN START */        
        {
            LOCK(cs_main);
            CNode* seed_node;
            if(!pto->fSyncedOnce && MercanteNode_IsBlockChainSynced(pto))
            {
