...
std::string HelpMessage(HelpMessageMode mode)                                   // MCHN
{
    ...
    strUsage += "  -conf=<file>           " + strprintf(_("Specify configuration file (default: %s)"), "mercante.conf") + "\n";
...
#ifndef WIN32
    strUsage += "  -pid=<file>            " + strprintf(_("Specify pid file (default: %s)"), "mercante.pid") + "\n";
#endif
    strUsage += "  -reindex               " + _("Rebuild the blockchain and reindex transactions on startup.") + "\n";
#if !defined(WIN32)
    strUsage += "  -sysperms              " + _("Create new files with system default permissions, instead of umask 077 (only effective with disabled wallet functionality)") + "\n";
    strUsage += "  -shortoutput           " + _("Returns connection string if this node can start or default mercante address otherwise") + "\n";
#endif
/* MCHN START */    
/* Default was 0 */    
...
//    strUsage += "  -blockprioritysize=<n> " + strprintf(_("Set maximum size of high-priority/low-fee transactions in bytes (default: %d)"), DEFAULT_BLOCK_PRIORITY_SIZE) + "\n";
    ...
    strUsage += "\n" + _("Mercante runtime parameters") + "\n";    
    strUsage += "  -offline                                 " + _("Start mercanted in offline mode, no connections to other nodes.") + "\n";
    strUsage += "  -initprivkey=<privkey>                   " + _("Manually set the wallet default address and private key when running mercanted for the first time.") + "\n";
    strUsage += "  -handshakelocal=<address>                " + _("Manually override the wallet address which is used for handshaking with other peers in a Mercante blockchain.") + "\n";
    ...
    strUsage += "\n" + _("Mercante API response parameters") + "\n";        
    strUsage += "  -hideknownopdrops=<n>  " + strprintf(_("Remove recognized Mercante OP_DROP metadata from the responses to JSON_RPC calls (default: %u)"), 0) + "\n";
    ...
    strUsage += "  -v1apicompatible=<n>   " + strprintf(_("JSON_RPC calls responses compatible with Mercante 1.0 (default: %u)"), 0) + "\n";
...
{
    return FormatParagraph(_("Copyright (c) Coin Sciences Ltd - www.mercante.com")) + "\n" +
           "\n" +
           ...
           "\n" +
           FormatParagraph(_("Full terms are shown at: http://www.mercante.com/terms-of-service/")) +
           "\n";
...
/* MCHN END */    
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Mercante Core is shutting down."));
...
// Anyway, we don't need this lock as we have permission db lock    
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Mercante Core is probably already running."), strDataDir));
#endif
/* MCHN END */
...
/* MCHN START */    
    if(mc_gState->m_NetworkParams->IsProtocolMercante())
    {
        LogPrintf("Mercante version build %s protocol %s (%s)\n", mc_BuildDescription(mc_gState->GetNumericVersion()), mc_gState->GetProtocolVersion(), CLIENT_DATE);
    }
...
/* MCHN START*/    
    uiInterface.InitMessage(_("Initializing mercante..."));
            ...
            if(mc_gState->m_NetworkParams->m_Status == MC_PRM_STATUS_MINIMAL)
            {
                ...
                if(init_privkey.size())
                {
                    if(seed_attempt == 1)
                    {
                        ...
                        {
                            return InitError(_("The initprivkey runtime parameter can only be used when connecting to Mercante 1.0 beta 2 or later"));                                                        
                        }
...
*/
            ...
            if(mc_QuerySeed(seedThreadGroup,seed_node))
            {
                ...
                {
                    seed_error="Couldn't disconnect from the seed node, please restart mercanted";
                }
                else
                {
//                    if(seed_port.size() == 0)
                    ...
                    {
                        seed_error=strprintf("Couldn't connect to the seed node %s on port %d - please check mercanted is running at that address and that your firewall settings allow incoming connections.",                
            ...
            if(mc_gState->m_NetworkParams->GetParam("protocolversion",&size) != NULL)
            {
                ...
                if(mc_gState->IsSupported(protocol_version) == 0) 
                {
                    ...
                    {
                        seed_error=strprintf("The protocol version (%d) for blockchain %s has been deprecated and was last supported in Mercante %s\n",                
                    ...
                    else
                    {
                        seed_error=strprintf("Couldn't connect to the seed node %s on port %d.\n"
                                    "Blockchain was created by mercanted with newer protocol version (%d)\n"                
                                    "Please upgrade to the latest version of Mercante or connect only to blockchains using protocol version %d or earlier.\n",                
            ...
            if(mc_gState->m_NetworkParams->m_Status == MC_PRM_STATUS_VALID)
            {
                SelectMercanteParams(mc_gState->m_Params->NetworkName());
        ...
        else
        {
            if(mc_gState->m_NetworkParams->m_Status == MC_PRM_STATUS_GENERATED)
            {
                ...
                mc_gState->m_NetworkParams->SetGlobals();                           // Needed to update IsProtocolMercante flag in case of bitcoin
    ...
    if(mc_gState->m_NetworkParams->m_Status == MC_PRM_STATUS_VALID)
    {
        ...
        {
            seed_error=strprintf("ERROR: Couldn't initialize asset database for blockchain %s. Please restart mercanted with reindex=1.\n",mc_gState->m_Params->NetworkName());
        ...
        else
        {
            ...
            if(mc_gState->m_WalletMode != MC_WMD_NONE)
            {
                ...
                {
                    return InitError("Wallet tx database corrupted. Please restart mercanted with -rescan\n");                        
                }
                ...
                {
                    return InitError(strprintf("Wallet tx database was created with different wallet version (%d). Please restart mercanted with reindex=1 \n",pwalletTxsMain->m_Database->m_DBStat.m_WalletVersion));                        
                }        

                if((pwalletTxsMain->m_Database->m_DBStat.m_InitMode & MC_WMD_MODE_MASK) != (mc_gState->m_WalletMode & MC_WMD_MODE_MASK))
                {
                    return InitError(strprintf("Wallet tx database was created in different mode (%08X). Please restart mercanted with reindex=1 \n",pwalletTxsMain->m_Database->m_DBStat.m_InitMode));                        
                }        
            }
        }
        ...
        if(pwalletMain == NULL)                                                 // Opening wallet only after mercante parameters were initizalized
        {
            pwalletMain = new CWallet(strWalletFile);
            DBErrors nLoadWalletRetForBuild = pwalletMain->LoadWallet(fFirstRunForBuild);
            ...
            if(pwalletMain)
            {
                ...
                if(pwalletMain->vchDefaultKey.IsValid())
                {
                    ...
                    {    
                        ...
                        sprintf(bufOutput,"mercante-cli %s grant %s connect\n",mc_gState->m_NetworkParams->Name(),
                             CBitcoinAddress(pwalletMain->vchDefaultKey.GetID()).ToString().c_str());
                        bytes_written=write(OutputPipe,bufOutput,strlen(bufOutput));
                        sprintf(bufOutput,"mercante-cli %s grant %s connect,send,receive\n\n",mc_gState->m_NetworkParams->Name(),
                             CBitcoinAddress(pwalletMain->vchDefaultKey.GetID()).ToString().c_str());
                        bytes_written=write(OutputPipe,bufOutput,strlen(bufOutput));
                    }
                    else
...
/* MCHN START */    
    ...
    if(!GetBoolArg("-shortoutput", false))
    {
        if(fListen && !GetBoolArg("-offline",false))
        {
            ...
            bytes_written=write(OutputPipe,bufOutput,strlen(bufOutput));
            sprintf(bufOutput,"mercanted %s:%d\n\n",MercanteServerAddress().c_str(),GetListenPort());
            bytes_written=write(OutputPipe,bufOutput,strlen(bufOutput));
            if(found_ips > 1)
            {
        ...
        else
        {
            if(GetBoolArg("-offline",false))
            {                
                sprintf(bufOutput,"Mercante started in offline mode, other nodes cannot connect.\n\n");
                bytes_written=write(OutputPipe,bufOutput,strlen(bufOutput));            
            }
            else
            {
...
/* MCHN START */                
                bool fRet = uiInterface.ThreadSafeMessageBox(
                    strLoadError + ".\n\n" + _("Please restart mercanted with reindex=1."),
...
//    int version=mc_gState->m_NetworkParams->GetInt64Param("protocolversion");
    ...
    LogPrintf("Mercante protocol version: %d\n",version);
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Mercante Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)                         // MCHN
            {
                strErrors << _("Wallet needed to be rewritten: restart Mercante Core to complete") << "\n";
