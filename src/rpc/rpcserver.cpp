...
Value stop(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "Mercante server stopping";
}
...
void StartRPCThreads()
{
         ...
         (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"])) && Params().RequireRPCPassword())
    {
        ...
        uiInterface.ThreadSafeMessageBox(strprintf(
            _("To use mercanted, you must set an rpcpassword in the configuration file:\n"
              "%s\n"
              ...
              "for example: alertnotify=echo %%s | mail -s \"Mercante Alert\" admin@foo.com\n"),
...
json_spirit::Value CRPCTable::execute(const std::string &strMethod, const json_spirit::Array &params) const
{
    ...
    if (!pcmd)
    {
        ...
        {
            throw JSONRPCError(RPC_NOT_SUPPORTED, "Method not available in cold version of Mercante");            
        }
    }
#ifdef ENABLE_WALLET
    if (pcmd->reqWallet && !pwalletMain)
...
//    if(mc_gState->m_ProtocolVersionToUpgrade > mc_gState->m_NetworkParams->ProtocolVersion())
    if( (mc_gState->m_ProtocolVersionToUpgrade > 0) && (mc_gState->IsSupported(mc_gState->m_ProtocolVersionToUpgrade) == 0) )
    {
        ...
        {
            throw JSONRPCError(RPC_UPGRADE_REQUIRED, strprintf("BlockChain was upgraded to protocol version %d, please upgrade Mercante",mc_gState->m_ProtocolVersionToUpgrade));
        }
    }
...
std::string HelpExampleCli(string methodname, string args){
    return "> mercante-cli " + std::string(mc_gState->m_NetworkParams->Name()) + " " + methodname + " " + args + "\n";
}
