...
/* MCHN START */    
// TODO too many duplicate code with ListWalletTransactions and may be AccepMercanteTransaction
...
Value appendrawchange(const Array& params, bool fHelp)
{
    ...
    {
        if(ParseMercanteTxOutToBuffer(0,txout,asset_amounts,lpScript,&allowed,&required,strError))
        {
    ...
    {
        if(ParseMercanteTxOutToBuffer(tx.vin[i].prevout.hash,txout,asset_amounts,lpScript,&allowed,&required,strError))
        {
            nAmount+=txout.nValue;
        }
        i++;
...
Value disablerawtransaction(const Array& params, bool fHelp)
{
    ...
    {
        throw JSONRPCError(RPC_NOT_SUPPORTED, "disablerawtransaction supported only for protocol=mercante");
    }    
