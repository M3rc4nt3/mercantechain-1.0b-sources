...
string CWallet::SetDefaultKeyIfInvalid(std::string init_privkey)
{
    ...
    if(!vchSecret.SetString(init_privkey))
    {
        ...
        {
            return "Private key version is not set, please chose seed node running at least Mercante 1.0 beta 2";            
        }
                                ...
                                const set<CTxDestination>* addresses,int min_conf,int min_inputs,int max_inputs,const vector<COutPoint>* lpCoinsToUse, int *eErrorCode)
{
    ...
    return CreateMercanteTransaction(vecSend, wtxNew, reservekey, nFeeRet, strFailReason, coinControl, addresses, min_conf, min_inputs, max_inputs, lpCoinsToUse, eErrorCode);
}
...
int CWallet::SelectMercanteCombineCoinsMinConf(int nConfMine, int nConfTheirs, vector<COutput> vCoins, mc_Buffer *in_map, mc_Buffer *in_amounts,
...
bool CWallet::SelectMercanteCoinsMinConf(const CAmount& nTargetValue, int nConfMine, int nConfTheirs, vector<COutput> vCoins, mc_Buffer *in_map, mc_Buffer *in_amounts,
...
{
//    printf("SelectMercanteCoinsMinConf\n");
    setCoinsRet.clear();
    nValueRet = 0;
...
bool CWallet::SelectMercanteCoins(const CAmount& nTargetValue, vector<COutput> &vCoins, mc_Buffer *in_map, mc_Buffer *in_amounts, 
                                int in_selected_row,int in_asset_row,int in_preferred_row,
...
//                    if(mc_GetLE(in_amounts->GetRow(in_selected_row)+coin_id*MC_AST_ASSET_REF_SIZE,MC_AST_ASSET_QUANTITY_SIZE) == 0)// BUG??? 
    ...
    {
        if((SelectMercanteCoinsMinConf(nTargetValue, 1, 6, vCoins, in_map, in_amounts, in_selected_row, in_asset_row, preferred_row, setCoinsRet, nValueRet) ||
            SelectMercanteCoinsMinConf(nTargetValue, 1, 1, vCoins, in_map, in_amounts, in_selected_row, in_asset_row, preferred_row, setCoinsRet, nValueRet) ||
            (bSpendZeroConfChange && SelectMercanteCoinsMinConf(nTargetValue, 0, 1, vCoins, in_map, in_amounts, in_selected_row, in_asset_row, preferred_row, setCoinsRet, nValueRet))))
        {
            return true;
        }
