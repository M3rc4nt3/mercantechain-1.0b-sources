...
int main(int argc, char* argv[])
{
    ...
    mc_MercanteParams* params;
    mc_MercanteParams* paramsOld;
...
//                    err=params->Create(mc_gState->m_Params->m_Arguments[1],version);
                ...
                { 
                    ...
                    printf("You can edit it in %s before running mercanted for the first time.\n\n",fileName);
                    printf("To generate blockchain please run \"mercanted %s -daemon\".\n",params->Name());
                }                
                else
                {
        ...
        if(strcmp(mc_gState->m_Params->Command(),"clone") == 0)
        {
            ...
            {
                params=new mc_MercanteParams;
                paramsOld=new mc_MercanteParams;
...
*/        
    ...
    if(err == MC_ERR_OPERATION_NOT_SUPPORTED)
    {
        ...
        printf("Usage:\n");
        printf("  mercante-util create <blockchain-name>  ( <protocol-version> = %d ) [options]        Creates new mercante configuration file %s with default parameters\n",
        ...
        printf("  mercante-util clone <old-blockchain-name> <new-blockchain-name> [options]               Creates new mercante configuration file %s copying parameters\n",fileName);
