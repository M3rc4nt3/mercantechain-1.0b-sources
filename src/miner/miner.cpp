            ...
            if(canMine)
            {
                ...
                *canMine=mc_gState->m_Permissions->CanMine(NULL,pubkey_hash);
                if((*canMine & MC_PTP_MINE) == 0)
                {
                    ...
                    {
                        LogPrintf("mchn: MercanteMiner: cannot mine now, waiting...\n");
                    }
                    testValidity=false;
                }
                else
                {
                    ...
                    {
                        LogPrintf("CreateNewBlock(): total size %u\n", nBlockSize);
                        LogPrintf("mchn: MercanteMiner: Starting mining...\n");
                    }                    
                }
            }            
        }
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        {
            return error("MercanteMiner : mining is paused, generated block is dropped");            
        }
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
        {
            return error("MercanteMiner : generated block is stale");
        }
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("MercanteMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("MercanteMiner started\n");
...
/* MCHN START */            
            ...
            if (Params().MiningRequiresPeers() 
                    ...
                    || (mc_gState->m_NetworkParams->IsProtocolMercante() == 0)
                    )
                    ) {
...
/* MCHN END */    
            if (!pblocktemplate.get())
            {
                LogPrintf("Error in MercanteMiner: Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce,pwallet);

            LogPrint("mcminer","mchn-miner: Running MercanteMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
//                        if(mc_gState->m_ProtocolVersionToUpgrade > mc_gState->m_NetworkParams->ProtocolVersion())
                        if( (mc_gState->m_ProtocolVersionToUpgrade > 0) && (mc_gState->IsSupported(mc_gState->m_ProtocolVersionToUpgrade) == 0) )
                        {
                            LogPrintf("MercanteMiner: Waiting for upgrade, block is dropped\n");
                        }
                        else
                        {
...
/* MCHN END */    
    ...
    {
        LogPrintf("MercanteMiner terminated\n");
        throw;
    }
}
