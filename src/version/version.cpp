...
int mc_State::VersionInfo(int version)
{
    ...
    if(version < MERCANTE_VERSION_CODE_MIN_VALID)
    {
        switch(version)
        {
            case MERCANTE_VERSION_CODE_BUILD:                                 // last version
                return -this_build;                                               
            case MERCANTE_VERSION_CODE_PROTOCOL_THIS:                         // last protocol version
                return this_protocol;
            case MERCANTE_VERSION_CODE_PROTOCOL_MIN:
                return 10004;                                                   // first supported version
            case MERCANTE_VERSION_CODE_PROTOCOL_MIN_DOWNGRADE:
                return 10008;                                                   // cannot downgrade below this version
            case MERCANTE_VERSION_CODE_PROTOCOL_MIN_NO_DOWNGRADE:
                return 20002;                                                   // if we are on this version or above, downgrades are forbidden
            case MERCANTE_VERSION_CODE_PROTOCOL_FOR_RELEVANCE:                
    ...
    return VersionInfo(MERCANTE_VERSION_CODE_BUILD)-1;                        // Created by the following builds
}
...
int mc_State::IsDeprecated(int version)
{
    ...
    if(ptr)
    {
        if(strcmp((char*)ptr,"mercante"))
        {
            is_protocol_mercante=0;
        }
    }
    
    if(is_protocol_mercante)
    {
        if(build > 0)
        {
...
{
    return -VersionInfo(MERCANTE_VERSION_CODE_BUILD);
}

int mc_State::GetProtocolVersion()
{
    return VersionInfo(MERCANTE_VERSION_CODE_PROTOCOL_THIS);
}

int mc_State::MinProtocolVersion()
{
    return VersionInfo(MERCANTE_VERSION_CODE_PROTOCOL_MIN);
}

int mc_State::MinProtocolDowngradeVersion()
{
    return VersionInfo(MERCANTE_VERSION_CODE_PROTOCOL_MIN_DOWNGRADE);
}

int mc_State::MinProtocolForbiddenDowngradeVersion()
{
    return VersionInfo(MERCANTE_VERSION_CODE_PROTOCOL_MIN_NO_DOWNGRADE);    
}

int mc_State::RelevantParamProtocolVersion()
{
    return VersionInfo(MERCANTE_VERSION_CODE_PROTOCOL_FOR_RELEVANCE);        
}
