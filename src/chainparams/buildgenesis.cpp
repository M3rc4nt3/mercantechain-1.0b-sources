...
int mc_GenerateConfFiles(const char *network_name)
{
    ...
    fileHan=mc_OpenFile(NULL,"mercante",".conf","r",MC_FOM_RELATIVE_TO_DATADIR | MC_FOM_CREATE_DIR);
    if(fileHan == NULL)
    {
        fileHan=mc_OpenFile(NULL,"mercante",".conf","w",MC_FOM_RELATIVE_TO_DATADIR);
    }
    if(fileHan)
    {
    ...
    fileHan=mc_OpenFile(network_name,"mercante",".conf","r",MC_FOM_RELATIVE_TO_DATADIR | MC_FOM_CREATE_DIR);
    if(fileHan == NULL)
    {
        fileHan=mc_OpenFile(network_name,"mercante",".conf","w",MC_FOM_RELATIVE_TO_DATADIR);
        if(fileHan)
        {
            fprintf(fileHan,"rpcuser=mercanterpc\n");
...
int mc_MercanteParams::Build(const unsigned char* pubkey, int pubkey_size) 
{
