...
int mc_OneMercanteParam::IsRelevant(int version)
{
    int ret=1;
...
void mc_MercanteParams::Destroy()
{
    if(m_lpData)
    {
        mc_Delete(m_lpData);
...
int64_t mc_MercanteParams::GetInt64Param(const char *param)
{
    int size;
...
double mc_MercanteParams::Int64ToDecimal(int64_t value)
{
    if(value < 0)
    {
...
int64_t mc_MercanteParams::DecimalToInt64(double value)
{
    return (int64_t)(value*MC_PRM_DECIMAL_GRANULARITY+mc_gState->m_NetworkParams->ParamAccuracy());    
}

int mc_MercanteParams::GetParamFromScript(char* script,int64_t *value,int *size)
{
    char *ptr;    
    ptr=script;
    ptr+=strlen(ptr)+1;
...
double mc_MercanteParams::GetDoubleParam(const char *param)
{
...
void* mc_MercanteParams::GetParam(const char *param,int* size)
{
    if(m_lpIndex == NULL)
    {
        return NULL;
...
int mc_MercanteParams::IsParamUpgradeValueInRange(const mc_OneMercanteParam *param,int version,int64_t value)
{
...
int mc_MercanteParams::CanBeUpgradedByVersion(const char *param,int version,int size)
{
    if(m_lpIndex == NULL)
    {
...
int mc_MercanteParams::SetParam(const char *param,const char* value,int size)
{
    int offset;
    if(m_lpIndex == NULL)
    {
...
int mc_MercanteParams::SetParam(const char *param,int64_t value)
{
    int size;
    char buf[8];
    if(m_lpIndex == NULL)
...
void mc_MercanteParams::Init()
{
    ...
    m_Count=sizeof(MercanteParamArray)/sizeof(mc_OneMercanteParam);
    max_size=0;
    
    for(i=0;i<m_Count;i++)
    {
        size=0;
        switch((MercanteParamArray+i)->m_Type & MC_PRM_DATA_TYPE_MASK)
        {
            case MC_PRM_BINARY  : size=(MercanteParamArray+i)->m_MaxStringSize;   break;
            case MC_PRM_STRING  : size=(MercanteParamArray+i)->m_MaxStringSize+1; break;
    ...
    m_lpParams=(mc_OneMercanteParam*)mc_New(sizeof(MercanteParamArray));
    m_lpData=(char*)mc_New(max_size);
    m_lpCoord=(int*)mc_New(2*m_Count*sizeof(int));
    
    memcpy(m_lpParams,MercanteParamArray,sizeof(MercanteParamArray));
    for(i=0;i<m_Count;i++)
    {
...
int mc_MercanteParams::Create(const char* name,int version)
{
    int size,offset,i,set;
    mc_OneMercanteParam *param;
    char *ptrData;
    int num_sets;
...
double mc_MercanteParams::ParamAccuracy()
{
    return 1./(double)MC_PRM_DECIMAL_GRANULARITY;
}


const mc_OneMercanteParam *mc_MercanteParams::FindParam(const char* param)
{
    int i;
    for(i=0;i<m_Count;i++)
    {
        if(strcmp((MercanteParamArray+i)->m_Name,param) == 0)
        {
            return MercanteParamArray+i;
        }        
    }    
    return NULL;
}

int mc_MercanteParams::Read(const char* name)
{
    return Read(name,0,NULL,0);
}

int mc_MercanteParams::Read(const char* name,int argc, char* argv[],int create_version)
{
    ...
    mc_MercanteParams *lpDefaultParams;
    ...
    lpDefaultParams = new mc_MercanteParams;
    ...
    for(i=0;i<m_Count;i++)
    {
        ...
        {        
            ...
            switch((MercanteParamArray+i)->m_Type & MC_PRM_DATA_TYPE_MASK)
            {
                ...
                case MC_PRM_INT32:
                    size=4;
                    if(((MercanteParamArray+i)->m_Type & MC_PRM_SOURCE_MASK) == MC_PRM_USER)
                    {
                        if(atoll(ptr) > (MercanteParamArray+i)->m_MaxIntegerValue)
                        {
                            printf("Invalid parameter value for %s - too high: %s\n",param->m_DisplayName,ptr);                        
                            return MC_ERR_INVALID_PARAMETER_VALUE;                                                
                        }
                        if(atoll(ptr) < (MercanteParamArray+i)->m_MinIntegerValue)
                        {
                            printf("Invalid parameter value for %s - too low: %s\n",param->m_DisplayName,ptr);                        
                            return MC_ERR_INVALID_PARAMETER_VALUE;                                                
                        }
                    }
                    if((MercanteParamArray+i)->m_Type & MC_PRM_DECIMAL)
                    {
                        double d=atof(ptr);
                        if(d >= 0)
                        {
        ...
        {
            if( ((((MercanteParamArray+i)->m_Type & MC_PRM_SOURCE_MASK) != MC_PRM_CALCULATED) && 
               (((MercanteParamArray+i)->m_Type & MC_PRM_SOURCE_MASK) != MC_PRM_GENERATED)) ||
               (argc > 0) )     
            {
                strcpy(m_lpData+offset,param->m_Name);
                offset+=strlen(param->m_Name)+1;
...
int mc_MercanteParams::Set(const char *name,const char *source,int source_size)
{
...
int mc_MercanteParams::Clone(const char* name, mc_MercanteParams* source)
{
    int err;
...
int mc_MercanteParams::CalculateHash(unsigned char *hash)
{
    int i;
    int take_it;
    mc_SHA256 *hasher;
...
int mc_MercanteParams::Validate()
{
    int i,size,offset;
    int isGenerated;
    int isMinimal;
    ...
    if(isValid)
    {
        ...
        else
        {
            ...
            if(strcmp((char*)protocol_name,"mercante") == 0)
            {
...
int mc_MercanteParams::Print(FILE* fileHan)
{
    int i,c,size;
    int version;
    int header_printed;
    ...
    fprintf(fileHan,"# ==== Mercante configuration file ====\n\n");
    fprintf(fileHan,"# Created by mercante-util \n");
    ...
    switch(m_Status)
    {
        case MC_PRM_STATUS_EMPTY:
            ...
            fprintf(fileHan,"# To join network please run \"mercanted %s@<seed-node-ip>[:<seed-node-port>]\".\n",Name());
            return MC_ERR_NOERROR;
        case MC_PRM_STATUS_ERROR:
            ...
            fprintf(fileHan,"# Please fix it and rerun mercante-util. \n");
            break;
        ...
        case MC_PRM_STATUS_GENERATED:
            ...
            fprintf(fileHan,"# To generate network please run \"mercanted %s\".\n",Name());
            break;
        case MC_PRM_STATUS_VALID:
            fprintf(fileHan,"# This parameter set is VALID. \n");
            fprintf(fileHan,"# To join network please run \"mercanted %s\".\n",Name());
            break;
    }
        ...
        while(i>=0)
        {
                    ...
                    ((m_lpParams+i)->IsRelevant(version) > 0))
            {
                ...
                if(header_printed == 0)
                {
                    ...
                    {
                        case MC_PRM_COMMENT:
                            fprintf(fileHan,"# The following parameters don't influence mercante network configuration. \n");
                        ...
                        case MC_PRM_USER:
                            if(m_Status == MC_PRM_STATUS_ERROR)
                            {
                                ...
                                if(Name())
                                {
                                    fprintf(fileHan,"# Please rerun \"mercante-util clone %s <new-network-name>\". \n",Name());
                                }
                            }
                            else
                            {
                                ...
                                {
                                    fprintf(fileHan,"# The following parameters can be edited before running mercanted for this chain. \n");                                    
                                }
                                else
                                {
                                    fprintf(fileHan,"# The following parameters can only be edited if this file is a prototype of another configuration file. \n");
                                    fprintf(fileHan,"# Please run \"mercante-util clone %s <new-network-name>\" to generate new network. \n",Name());
                                }
                            }
                            break;
                        case MC_PRM_GENERATED:
                            fprintf(fileHan,"# The following parameters were generated by mercante-util.\n");
                            fprintf(fileHan,"# They SHOULD ONLY BE EDITED IF YOU KNOW WHAT YOU ARE DOING. \n");
                            break;
                        case MC_PRM_CALCULATED:
                            fprintf(fileHan,"# The following parameters were generated by mercanted.\n");
...
int mc_MercanteParams::Write(int overwrite)
{
    FILE *fileHan;
    int create;
...
const char* mc_MercanteParams::Name()
{
    return (char*)GetParam("chainname",NULL);
}

const unsigned char* mc_MercanteParams::MessageStart()
{
    return (unsigned char*)GetParam("networkmessagestart",NULL);
}

const unsigned char* mc_MercanteParams::DefaultMessageStart()
{
    return c_DefaultMessageStart;
}


const unsigned char* mc_MercanteParams::AddressVersion()
{
    return (unsigned char*)GetParam("addresspubkeyhashversion",NULL);
}

const unsigned char* mc_MercanteParams::AddressScriptVersion()
{
    return (unsigned char*)GetParam("addressscripthashversion",NULL);
}

const unsigned char* mc_MercanteParams::AddressCheckumValue()
{
    return (unsigned char*)GetParam("addresschecksumvalue",NULL);
}


int mc_MercanteParams::ProtocolVersion()
{
...
int mc_MercanteParams::IsProtocolMercante()
{
    return m_IsProtocolMercante;
}
...
int mc_Features::VerifySizeOfOpDropElements()                                   // This test is still in the code to keep protocol!-mercante untouched
{
    
    int ret=0;        
    int protocol=mc_gState->m_NetworkParams->ProtocolVersion();
    
    if(mc_gState->m_NetworkParams->IsProtocolMercante() == 0)
    {
        return 1;
    }
