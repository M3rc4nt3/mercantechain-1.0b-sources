...
/* MCHN START */
static Checkpoints::MapCheckpoints mapCheckpointsMercante =
        boost::assign::map_list_of
        ( -1, uint256("0f9188f13cb7b2c71f2a335e3a4fc328bf5beb436012afca590b1a11466e2206"))
        ;
static const Checkpoints::CCheckpointData dataMercante = {
        &mapCheckpointsMercante,
        0,
        0,
        0
    };
...
class CMercanteParams : public CMainParams {
public:
    CMercanteParams() {    };
    
    void SetFixedParams(const char *NetworkName)
    {
        networkID = CBaseChainParams::MERCANTE;
...
//        convertSeed6(vFixedSeeds, pnSeed6_test, ARRAYLEN(pnSeed6_test));
    ...
    void SetMercanteParam(const char*param_name,int64_t value)
    {
...
static CMercanteParams MercanteParams;


bool SelectMercanteParams(const char *NetworkName)
{
    SelectMercanteBaseParams(NetworkName,(int)mc_gState->m_NetworkParams->GetInt64Param("defaultrpcport"));

    MercanteParams.SetFixedParams(NetworkName);
    
    MercanteParams.Initialize();
    
    pCurrentParams = &MercanteParams;
...
void SetMercanteParams()
{
    MercanteParams.SetMercanteParams();
}

void SetMercanteParam(const char*param_name,int64_t value)
{
    MercanteParams.SetMercanteParam(param_name,value);
}


void SetMercanteRuntimeParams()
{
    MercanteParams.SetMercanteRuntimeParams();
}

bool InitializeMercanteParams()
{
    
    MercanteParams.Initialize();
    
    MercanteParams.SetGenesis();
