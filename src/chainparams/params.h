...
#ifndef MERCANTEPARAMS_H
#define	MERCANTEPARAMS_H
...
typedef struct mc_OneMercanteParam
{    
...
typedef struct mc_MercanteParams
{    
    char *m_lpData;
    mc_MapStringIndex *m_lpIndex;
    mc_OneMercanteParam *m_lpParams;
    int *m_lpCoord;
    int m_Status;
    int m_Size;
    int m_Count;
    int m_IsProtocolMercante;
    int m_ProtocolVersion;
    ...
    int Clone(const char *name,mc_MercanteParams *source);
    ...
    const mc_OneMercanteParam *FindParam(const char *param);
    ...
    int IsParamUpgradeValueInRange(const mc_OneMercanteParam *param,int version,int64_t value);
