...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Mercante
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Mercante
    // Mac and Unix: ~/.mercante
#ifdef WIN32
    // Windows
    if(mc_gState->m_SessionFlags & MC_SSF_COLD)
    {
        return GetSpecialFolderPath(CSIDL_APPDATA) / "MercanteCold";
    }
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Mercante";
#else
    // Mac and Unix
    ...
    if(mc_gState->m_SessionFlags & MC_SSF_COLD)
    {
        return pathRet / ".mercante-cold";        
    }
    return pathRet / ".mercante";
#endif
}
...
void mc_CheckDataDirInConfFile()
{
    ...
    mapConfig=new mc_MapStringString;
    if(mc_ReadGeneralConfigFile(mapConfig,NULL,"mercante",".conf") == 0)
    {
...
{
    mc_ReadConfigFile(mc_GetConfigFile(network_name,"mercante",".conf"),&mapArgs, &mapMultiArgs,"-");    
...
int mc_MercanteParams::SetProtocolGlobals()
{
...
int mc_MercanteParams::SetGlobals()
{
...
int mc_MercanteParams::Import(const char *name,const char *source_address)
{
    
    return MC_ERR_NOERROR;
}

std::string MercanteServerAddress()
{
